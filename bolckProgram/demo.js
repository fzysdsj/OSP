 $("#bankerAlgorithm").on('click', function () {
        ...
        if (number % 2 != 0) {
            ...
            $('#equipmentEntering').click(function () {
                ...
                $('#equipmentInformation').append("*******华丽丽的开始线*********<br>");
                let lFlag = false;
                ...
                if (Need[processNumber].A < aNumber || Need[processNumber].B < bNumber || Need[processNumber].C < cNumber) {
                    ...
                    if (AVAILABLE.A < aNumber || AVAILABLE.B < bNumber || AVAILABLE.C < cNumber) {
                        ...
                        $('#equipmentInformation').append("请求资源合理,开始验证是否存在安全序列。<br>");
                        $('#equipmentInformation').append("运算中.......<br>");
                        ...
                        var timer = setInterval(function () {
                            ...
                            if (safeList.length == NeedDemo.length) {
                                gFlag = true;
                                clearInterval(timer);
                                $('#equipmentResult').append("<br>");
                                $('#equipmentResult').append("存在安全序列:");
                                for (let i = 0; i < safeList.length; i++) {
                                    if (i == safeList.length - 1) {
                                        $('#equipmentResult').append("进程" + safeList[i]);
                                    } else {
                                        $('#equipmentResult').append("进程" + safeList[i] + "→");
                                    }
                                }
                                ...
                                for (let i = 0; i < ProcessNumber.length; i++) {
                                    $('#equipmentResult').append("进程" + i + ",资源A：" + ProcessNumber[i].A + ",资源B：" + ProcessNumber[i].B + ",资源C：" + ProcessNumber[i].C + "<br>");
                                }
